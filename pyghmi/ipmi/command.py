...
class Command(object):
    ...
    def set_channel_access(self, channel=1, access_update_mode='non_volatile',
    ...
    def get_channel_access(self, channel=1, read_mode='non_volatile',  **kwargs_conn):
        """Get channel access
    ...
    def get_channel_info(self, channel=1):
        """Get channel info
    ...
    def set_user_access(self, uid, channel=1, callback=False, link_auth=True, ipmi_msg=True,
                        privilege_level='administrator'):
        """Set user access
        ...
        data = [b, uid & 0b00111111, privilege_levels[privilege_level] & 0b00001111]
        response = self.raw_command(netfn=0x06, command=0x43, data=data)
        if 'error' in response:
            raise Exception(response['error'])
        return True

    def get_user_access(self, uid, channel=1):
        """Get user access
        ...
        access:
            ...
            link_auth
            ipmi_msg
            privilege_level: [reserved, callback, user, operatorm administrator, proprietary, no_access]
        """
    ...
    def get_channel_max_user_count(self, channel=1):
    ...
    def get_user(self, uid, channel=1):
        """Get user (helper)
    ...
    def get_name_uids(self, name, channel=1):
    ...
    def ensure_user(self, uid, name, password, channel=1, callback=False,
                    link_auth=True, ipmi_msg=True, privilege_level='administrator'):
        """ensure a user is created with provided settings (helper)
        ...
        :param privilege_level:
            ...
            * no_access
        """
        # dont disable user untill settings are set because the current user might be trying to update
        # set_user_password(uid, password, mode='disable')
        self.set_user_name(uid, name)
        self.set_user_access(uid, channel, callback=callback,link_auth=link_auth, ipmi_msg=ipmi_msg,
                             privilege_level=privilege_level)
        self.set_user_password(uid, password=password)
        self.set_user_password(uid, mode='enable', password=password)
        return True

    def user_delete(self, uid, channel=1):
        """Delete user (helper)
        ...
        ## TODO: perhaps should set user access on all channels so new users dont get extra access
        self.set_user_access(uid, channel=channel, callback=False, link_auth=False, ipmi_msg=False,
