...
class ServerSession(session.Session):
    ...
    def __init__(self, authdata, kg, clientaddr, netsocket, request, uuid):
        ...
        self.uuid = uuid
        self.rqaddr = constants.IPMI_BMC_ADDRESS # spec says rqaddr for bmc should be 20h
    ...
    def _got_rakp3(self, data):
        ...
        # respond correctly a TODO, since Kg being used
        ...
        if expectedauthcode != authcode:
            #TODO: RMCP error back at invalid rakp3
            return
        clienttag = data[0]
        if data[1] != 0:
    ...
    def handle_client_request(self, request):
        if request['netfn'] == 6:
            self._handle_app_request(request)
        else:
            print "got a request: " + repr(request)

    def _handle_app_request(self, request):
        # this handles the class of requests that are netfn== application
        if request['command'] == 0x3b: # set session priv level
            print 'ok... ' + repr(request)
            if request['data'][0] > 1:
                self.clientpriv = request['data'][0]
            self._send_ipmi_net_payload(
                netfn=7, command=0x3b, data=[self.clientpriv])
        else:
            print repr(request)
