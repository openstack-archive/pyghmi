...
class Session:
    ...
    def wait_for_rsp(cls, timeout=None):
        ...
        if len(rdylist) > 0:
            while len(_poller((cls.socket,))) > 0:  # if the somewhat lengthy
                        # queue # processing takes long enough for packets to
                        # come in, be eager
                pktqueue = collections.deque([])
                while len(_poller((cls.socket,))) > 0:  # looks rendundant, but
                ...
                while len(pktqueue):
                    ...
                    while len(_poller((cls.socket,))) > 0:  # seems ridiculous,
