...
try:
    ...
    def _encrypt(key, iv, data):
        crypter = ciphers.Cipher(ciphers.algorithms.AES(key),
                                 cipers.modes.CBC(iv), backend=cryptbackend
                                 ).encryptor()
        return crypter.update(data) + crypter.finalize()


    def _decrypt(key, iv, data):
        decrypter = ciphers.Cipher(ciphers.algorithms.AES(key),
                                   ciphers.modes.CBC(iv), backend=cryptbackend
                                   ).decryptor()
