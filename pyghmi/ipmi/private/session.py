...
class Session(object):
    ...
    def __init__(self,
        ...
        if hasattr(self, 'initialized'):
            ...
            #TODO(jbjohnso): a better solution would be to
            #extend wait_for_rsp to just call the callback
            #next go around, if there is a callback
            return
    ...
    def _keepalive(self):
        ...
        """
        if self.incommand: #if currently in command, no cause to keepalive anyway
            return
    ...
    def _xmit_packet(self, retry=True, delay_xmit=None):
        if not self.nowait:  # if we are retrying, we really need to get the
            ...
            Session.wait_for_rsp(timeout=0, callout=False)  # take a convenient opportunity
