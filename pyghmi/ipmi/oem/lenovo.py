        ...
        if self.is_fpc:
            for sensorname in nextscale_fpc_sensors:
                ...
                if 'elements' in sensor:
                    ...
                    yield {'name': sensorname, 'type': sensor['type']}
    ...
    def get_sensor_reading(self, sensorname):
        if self.is_fpc:
            ...
            if (sensorname in nextscale_fpc_sensors and
                ...
                sensor = nextscale_fpc_sensors[sensorname]
                value = sensor['provider'](self.ipmicmd)

            else:
                sensortypename, _, sensoridx = sensorname.rpartition(' ')
                sensoridx = int(sensoridx)
                nfs = nextscale_fpc_sensors
                if (sensortypename in nfs and
                        sensoridx <= nfs[sensortypename]['elements']):
                    sensor = nextscale_fpc_sensors[sensorname]
                    value = sensor['provider'](self.ipmicmd, sensoridx)
            if sensor is not None:
                return sdr.SensorReading(
                    {'name': sensorname, 'imprecision': None,
                     'value': value, 'states': [], 'health': 0,
                     'type': sensor['type']}, sensor['units'])
