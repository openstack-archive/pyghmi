...
def parse_inventory_category(name, info):
    ...
    # TODO: raise specific exception to point that there's data left in the
    # buffer
    if cur != len(raw): raise Exception
    # TODO: raise specific exception to point that the number of entries is
    # different than the expected
    if count != len(entries): raise Exception
    return entries
...
def parse_inventory_category_entry(raw, fields):
    ...
    for field in fields:
        ...
        if (field.fmt[-1] == "s"): value = value.rstrip("\x00")
