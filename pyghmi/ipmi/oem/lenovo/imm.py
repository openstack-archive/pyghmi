...
class XCCClient(IMMClient):
    ...
    def update_firmware(self, filename, data=None):
        try:
            self.update_firmware_backend(filename, data)
        except Exception:
            self.wc.grab_json_response('/api/providers/fwupdate', json.dumps(
                {'UPD_WebCancel': 1}))
            raise

    def update_firmware_backend(self, filename, data=None):
        ...
        while uploadthread.isAlive():
            ...
            print(repr(self.wc.grab_json_response('/api/providers/identity')))
