...
class OEMHandler(generic.OEMHandler):
    ...
    def _megarac_attach_media(self, proto, username, password, imagename,
        ...
        self.ipmicmd.xraw_command(netfn=0x32, command=0x9f, data=(8, 10, 0, 0))
        ...
        while imageretries:
            try:
                # first stop one if it is in a bad state
                ...
                if imageretries == 1:
                    raise
                if ie.ipmicode in (0x92, 0x95, 0x99, 0x96):
                    imageretries = imageretries - 1
                    self.ipmicmd.wait_for_rsp(2)
                elif ie.ipmicode == 0x95:  # slot busy... detach
                    self.ipmicmd.xraw_command(netfn=0x32, command=0xa0,
                                          data=(1, 0))
                    self.ipmicmd.wait_for_rsp(2)
                else:
