...
class FFDCFetcher(object):
    ...
    def fetch_ffdc_file(self):
        ...
        # use tmp folder to store the ffdc log file
        timestamp = bytes(time.time()).split('.')[0]
        filename = 'tmp/ffdc_' + timestamp + '.tgz'
        dir_name = os.path.dirname(filename)

        if not os.path.exists(dir_name):
            os.makedirs(dir_name)

        # clean previous ffdc files
        filelist = [f for f in os.listdir(dir_name) if f.startswith('ffdc')]
        try:
            for f in filelist:
                os.remove(f)
        except Exception:
