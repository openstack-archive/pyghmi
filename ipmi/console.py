...
class Console(object):
    ...
    def _payload_activated(self, response):
        ...
        if (response['code']):
            if response['code'] in constants.ipmi_completion_codes:
                self._print_data(constants.ipmi_completion_codes[response['code']])
                return
    ...
    def _got_cons_input(self,handle):
        ...
        """
        print 'TODO(jbjohnso): craft sol send payload got '+repr(handle.read())
    ...
    def _got_sol_payload(self, payload):
        ...
        """
        #TODO(jbjohnso) test cases to throw some likely scenarios at the functions
        ...
        if newseq != 0: #this packet at least has some data to send to us..
            ...
            self.ipmi_session.send_payload(ackpayload, payload_type=1)
